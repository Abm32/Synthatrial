...
def test_drug_processing():
    ...
    for smiles, name in test_cases:
        try:
            ...
            assert len(fingerprint) == 2048, f"Fingerprint length should be 2048, got {len(fingerprint)}"
            assert all(isinstance(x, (int, float)) for x in fingerprint), "Fingerprint should contain numbers"
...
def test_cyp2d6_poor_metabolizer():
    ...
    try:
        ...
        has_risk_mention = any(word in result_lower for word in ['risk', 'high', 'low', 'medium', 'adverse'])
        ...
        if has_risk_mention or has_cyp_mention:
            ...
            print(f"  ✓ Output mentions risk/CYP: {has_risk_mention or has_cyp_mention}")
            print(f"  Result preview: {result[:200]}...")
            return True
        else:
            print(f"  ⚠ {codeine['name']} simulation completed but output format unexpected")
