...
def extract_risk_level(prediction: str) -> str:
    ...
    if 'high' in prediction_lower and ('risk' in prediction_lower or 'severe' in prediction_lower):
        return 'High'
    elif 'medium' in prediction_lower and ('risk' in prediction_lower or 'moderate' in prediction_lower):
        return 'Medium'
...
def run_validation_suite() -> Dict:
    ...
    for result in results:
        ...
        print(f"  {status} {result['drug']}: Expected {result['expected_risk']}, Got {result['actual_risk']}")
...
def generate_paper_summary(validation_stats: Dict):
    ...
    print(f"Correct Predictions: {validation_stats['correct_predictions']}/{validation_stats['total_cases']}")
