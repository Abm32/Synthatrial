...
def extract_drug_molecules(conn: sqlite3.Connection, limit: Optional[int] = None) -> List[Dict]:
    """
...
def extract_drug_targets(conn: sqlite3.Connection, molregno: int) -> List[Dict]:
    ...
    targets = []
    for row in cursor.fetchall():
        tid, pref_name, organism, target_type, std_type, std_value, std_units, std_relation = row
        targets.append({
            'tid': tid,
            'name': pref_name,
            'organism': organism,
...
def prepare_drug_for_vector_db(drug: Dict, conn: sqlite3.Connection) -> Optional[Dict]:
    ...
    try:
        ...
        metadata = {
            ...
            'known_side_effects': '; '.join(side_effects) if side_effects else 'None listed',
...
def batch_extract_drugs(
    ...
    try:
        ...
        print(f"Successfully prepared {len(vector_db_records)} drugs for vector database")
