...
from src.agent_engine import run_simulation, extract_risk_level
...
# Load Animations
lottie_dna = load_lottieurl("https://lottie.host/80706597-2858-4504-8b89-138332994c63/E8e4Xg6KqE.json") # Example DNA animation
...
with st.sidebar:
    ...
    api_url = st.text_input("API URL", value="http://localhost:8000", help="URL of the backend API")
...
with col1:
    st.markdown('<div class="hero-title">Virtual Clinical Trials</div>', unsafe_allow_html=True)
    st.markdown('<div class="hero-subtitle">Simulate drug interactions and pharmacogenomics risks with AI. Reduce trial costs and improve safety.</div>', unsafe_allow_html=True)
with col2:
    if lottie_dna:
        st_lottie(lottie_dna, height=150)

# Tabs
tab1, tab2, tab3, tab4 = st.tabs(["🧪 Simulation Lab", "📦 Batch Processing", "📊 Analytics", "ℹ️ About"])
...
with tab1:
    ...
    with col_input:
        ...
        with st.container():
            ...
            # Drug Selection
            drug_source = st.radio("Drug Source", ["Standard Library", "Custom Molecule/SMILES"], horizontal=True)
            
            if drug_source == "Standard Library":
                drug_name = st.selectbox("Select Drug", ["Warfarin", "Clopidogrel", "Codeine", "Ibuprofen", "Metoprolol", "Simvastatin", "Irinotecan"])
            ...
            c1, c2 = st.columns(2)
            with c1:
                cyp2d6 = st.selectbox("CYP2D6 Status", ["Extensive Metabolizer (Normal)", "Poor Metabolizer", "Intermediate Metabolizer", "Ultrarapid Metabolizer"])
                cyp2c9 = st.selectbox("CYP2C9 Status", ["Extensive Metabolizer (Normal)", "Poor Metabolizer", "Intermediate Metabolizer"])
            with c2:
                cyp2c19 = st.selectbox("CYP2C19 Status", ["Extensive Metabolizer (Normal)", "Poor Metabolizer", "Intermediate Metabolizer", "Ultrarapid Metabolizer"])
                ugt1a1 = st.selectbox("UGT1A1 Status", ["Extensive Metabolizer (Normal)", "Poor Metabolizer (*28/*28)", "Intermediate Metabolizer (*1/*28)"])
            
            slco1b1 = st.selectbox("SLCO1B1 Transporter Function", ["Normal Function", "Decreased Function", "Poor Function"])
...
with tab4:
    ...
    **Features:**
    - 🧬 **Genomic Integration**: VCF parsing and variant calling (chr22, chr10, chr2, chr12).
