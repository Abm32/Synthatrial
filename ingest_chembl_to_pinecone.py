...
def main():
    ...
    if not db_path:
        ...
        print("\nTo download ChEMBL:")
        print("  curl -L https://ftp.ebi.ac.uk/pub/databases/chembl/ChEMBLdb/releases/chembl_34/chembl_34_sqlite.tar.gz -o data/chembl/chembl_34_sqlite.tar.gz")
        sys.exit(1)
    ...
    for i in range(0, len(vector_records), batch_size):
        ...
        try:
            ...
            print(f"  Ingested batch {i//batch_size + 1}: {total_ingested}/{len(vector_records)} drugs")
        except Exception as e:
